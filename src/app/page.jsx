...
export default function App() {
  ...
  const withTimeout = useCallback(async(label, queryPromise, ms = 12000) => {
    let timer
    try {
  ...
  const loadAll = useCallback(async()=>{
    ...
    try {
      ...
      const dailyResult = d.status==="fulfilled" ? d.value : null
      const weeklyResult = w.status==="fulfilled" ? w.value : null

      if(t.status==="rejected") errors.push(t.reason?.message || "Trades failed to load")
      if(d.status==="rejected") errors.push(d.reason?.message || "Daily plans failed to load")
      if(w.status==="rejected") errors.push(w.reason?.message || "Weekly plans failed to load")
      if(tradesResult?.error) errors.push(`Trades: ${tradesResult.error.message}`)
      if(dailyResult?.error) errors.push(`Daily plans: ${dailyResult.error.message}`)
      if(weeklyResult?.error) errors.push(`Weekly plans: ${weeklyResult.error.message}`)
