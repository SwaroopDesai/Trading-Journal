    ...
    >
      ...
      <nav className="sidebar" style={{background:T.surface,borderRight:`1px solid ${T.border}`}}>
        <div style={{padding:"24px 20px 18px"}}>
          <div style={{fontFamily:"var(--font-geist-sans)",fontSize:22,fontWeight:900,color:T.accentBright,letterSpacing:"-0.06em"}}>FXEDGE</div>
        ...
        <button onClick={signOut} style={{margin:"6px 12px 18px",padding:"9px 14px",background:"none",border:`1px solid ${T.border}`,color:T.muted,borderRadius:8,cursor:"pointer",fontSize:12,fontFamily:"var(--font-geist-sans)",textAlign:"center",fontWeight:700,transition:"background .15s,color .15s"}} onMouseEnter={e=>{e.currentTarget.style.background=T.surface2;e.currentTarget.style.color=T.text}} onMouseLeave={e=>{e.currentTarget.style.background="none";e.currentTarget.style.color=T.muted}}>Sign Out</button>
      </nav>
...
function buildCSS(T) {
  ...
  return `
    ...
    * { box-sizing:border-box; margin:0; padding:0; }
    body { background:${T.bg}; font-family:var(--font-geist-sans); }
    ...
    .ambient-glow{position:fixed;width:50vw;height:70vh;filter:blur(80px);pointer-events:none;z-index:0;opacity:.16;}
    .ambient-glow-a{top:-26vh;left:-18vw;background:radial-gradient(circle, rgba(99,102,241,.72), transparent 64%);animation:driftA 22s ease-in-out infinite;}
    .ambient-glow-b{right:-22vw;bottom:-34vh;background:radial-gradient(circle, rgba(236,72,153,.62), transparent 66%);animation:driftB 25s ease-in-out infinite;}
    ...
    h1, h2, h3 { font-family: var(--font-geist-sans) !important; letter-spacing: -0.02em !important; }
