...
const DAILY_PAIR_NOTES_MARKER = "__FXEDGE_PAIR_NOTES__::";
...
function CardTitle({T,children,meta}) {
  return (
...
function SectionLead({T,eyebrow,title,copy,action,compact}) {
  return (
...
const serializeDailyPairNotes = (pairNotes) => {
  const cleaned = Object.fromEntries(Object.entries(pairNotes || {}).filter(([,value])=>String(value || "").trim()))
...
function Analytics({T,stats,trades,viewportWidth}) {
  ...
  const isMobile = viewportWidth < 768
  const bestSession = [...stats.bySession].sort((a,b)=>b.totalR-a.totalR)[0]
  ...
  return (
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:isMobile?"1fr":"repeat(2,minmax(0,1fr))",gap:14}}>
        <Card T={T}><CardTitle T={T} meta={stats.bySession.length?`${stats.bySession.length} active buckets`:"Waiting on trades"}>By Session</CardTitle>{stats.bySession.length===0?E:stats.bySession.map(s=><BarRow key={s.session} label={s.session.split("/")[0]} wins={s.wins} count={s.count} totalR={s.totalR} color={T.green}/>)}</Card>
...
function TradeModal({T,userId,initial,onSave,onClose,syncing}) {
  const blank={pair:"EURUSD",date:new Date().toISOString().split("T")[0],direction:"LONG",session:"London",killzone:"",dailyBias:"Bullish",weeklyBias:"",marketProfile:"",manipulation:"Liquidity Sweep Low",poi:"Order Block",setup:"Manipulation + POI",entry:"",sl:"",tp:"",result:"WIN",rr:"",pips:"",emotion:"Calm & Focused",mistakes:"None",notes:"",preScreenshot:"",postScreenshot:"",tags:""}
  ...
  const submit=()=>{
    ...
    onSave({...f,killzone:"",weeklyBias:"",marketProfile:"",rr,pips:parseFloat(f.pips)||0,entry:parseFloat(f.entry)||0,sl:parseFloat(f.sl)||0,tp:parseFloat(f.tp)||0,tags:f.tags?f.tags.split(",").map(t=>t.trim()).filter(Boolean):[]})
  }
  const cancelDraft = ()=>{
